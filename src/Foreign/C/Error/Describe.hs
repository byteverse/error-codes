...
enumString (Errno i) = if fromIntegral i < (256 :: Word)
  then asString (PM.indexUnliftedArray enumTable (fromIntegral i))
  else unknownString

enumByteArray :: Errno -> ByteArray
enumByteArray (Errno i) = if fromIntegral i < (256 :: Word)
...
descriptionString (Errno i) = if fromIntegral i < (256 :: Word)
  then asString (PM.indexUnliftedArray descriptionTable (fromIntegral i))
  else unknownString

descriptionByteArray :: Errno -> ByteArray
descriptionByteArray (Errno i) = if fromIntegral i < (256 :: Word)
